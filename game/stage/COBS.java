...
public class COBS extends Stage {
    ...
    public COBS() {
        ...
        platforms.add(new MovingPlatform(576 + (int) plat1Pos.x, (int) plat1Pos.y, 280, 72, false,
                "ruined_platform_1.png", this));
        ...
        platforms.add(new MovingPlatform(576 + (int) plat2Pos.x, (int) plat2Pos.y, 280, 72, false,
                "ruined_platform_2.png", this));
        ...
        platforms.add(new MovingPlatform(576 + (int) plat3Pos.x, (int) plat3Pos.y, 280, 72, false,
                "ruined_platform_3.png", this));
    ...
    public void preUpdate() {
        if (Settings.stageHazards()) {
            ...
            if (--fastMode > 0 && battleScreen.getGameTick() % 15 == 0) {
                ...
                if (Math.random() > 0.5) {
                    spawnPos = new Vector(
                            Math.random() > 0.5 ? battleScreen.getStage().getUnsafeBlastZone().x
                                    : battleScreen.getStage().getUnsafeBlastZone().getX2(),
                            Math.random() * battleScreen.getStage().getUnsafeBlastZone().height
                                    + battleScreen.getStage().getUnsafeBlastZone().y);
                } else {
                    spawnPos = new Vector(
                            Math.random() * battleScreen.getStage().getUnsafeBlastZone().width
                                    + battleScreen.getStage().getUnsafeBlastZone().x,
                            Math.random() > 0.5 ? battleScreen.getStage().getUnsafeBlastZone().y
                                    : battleScreen.getStage().getUnsafeBlastZone().getY2());
                }
                Player target = Math.random() > 0.5 ? battleScreen.getPlayers(0) : battleScreen.getPlayers(1);
                battleScreen.addProjectileAtCenter(new GodEye(spawnPos.x, spawnPos.y,
                        Vector.normalize(Vector.sub(target.center(), spawnPos)).mul(50).x,
                        Vector.normalize(Vector.sub(target.center(), spawnPos)).mul(50).y, 10, 10,
                        stagePlayer.playerId, stagePlayer));
            }
        }
