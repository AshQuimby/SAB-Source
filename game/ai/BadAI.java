...
public class BadAI extends AI {
   ...
   ArrayList<Vector> targetPastPos = new ArrayList<Vector>();
   ...
   public void update(Player player) {
      ...
      targetPastPos.add(target.hitbox.getCenter());
      if (targetPastPos.size() > 30) {
         targetPos = targetPastPos.get(0);
         targetPastPos.remove(0);
      }
      ...
      if (target != null && targetPos != null) {
         ...
         if ((targetPos.y < player.center().y - 32)
               && player.velocity.y > 0) {
            if (player.jumps > 0 && Math.random() > 0.9) {
               tap(Player.JUMP);
            } else if (!player.falling && Math.random() > 0.9) {
               tap(Player.UP);
               tap(Player.ATTACK);
            }
         }
         ...
         if (targetProjectile != null && nextTargetProjectileFrame.overlaps(player.hitbox) && Math.random() > 0.9) {
            tap(Player.PARRY);
         }
         if (targetProjectile != null && projectileDistance < 96) {
            if (player.jumps > 0 && Math.random() > 0.9) {
               tap(Player.JUMP);
            } else if (!player.falling && projectileDistance < 48 && Math.random() > 0.9) {
               tap(Player.UP);
               tap(Player.ATTACK);
            }
         }
...
// nearestPlatform(player.battleScreen.getStage().getPlatforms());
// Projectile targetProjectile =
// nearestEnemyProjectile(player.battleScreen.getProjectiles());
// double distance = Vector.distanceBetween(target.center(), player.center());
// Vector nearestPlatformPoint =
// targetPlatform.getHitbox().nearestPointTo(player.hitbox.getCenter());
// Vector platformDistances = new Vector(Math.abs(nearestPlatformPoint.x -
// player.center().x),
// Math.abs(nearestPlatformPoint.y - player.center().y));
// double projectileDistance = 25600;
//
// Platform nearestPlatformBelow =
// nearestPlatformBelow(player.battleScreen.getStage().getPlatforms());
//
// Platform nearestPlatformToTarget = AI.nearestPlatform(target,
// player.battleScreen.getStage().getPlatforms());
//
// Vector targetPointOnPlatform =
// targetPlatform.getHitbox().nearestPointTo(target.center());
//
// Vector targetNearestPlatformPoint =
// nearestPlatformToTarget.getHitbox().nearestPointTo(target.center());
//
...
// if (targetProjectile != null) {
// projectileDistance =
// Vector.distanceBetween(targetProjectile.hitbox.getCenter(), player.center());
// nextTargetProjectileFrame = targetProjectile.hitbox.copy();
// if (targetProjectile.velocity != null) {
// nextTargetProjectileFrame.setPosition(Vector.add(targetProjectile.pos,
// targetProjectile.velocity));
// }
// }
// if (target != null && targetPos != null) {
// release(Player.DOWN);
...
// release(Player.PARRY);
//
// boolean platformScan = new CollisionLine(player.center().x,
// player.center().y, 2560,
// Math.toRadians(90)).collidingWithHitboxes(player.battleScreen.getStage().getHitboxes());
//
// boolean targetPlatformScan = new CollisionLine(target.center().x,
// target.center().y, 2560,
// Math.toRadians(90)).collidingWithHitboxes(player.battleScreen.getStage().getHitboxes());
//
// Ledge targetLedge = nearestLedge(player.battleScreen.getStage().ledges);
//
// if (!targetPlatformScan && (player.jumps < 1 || player.falling)) {
// targetPos = new Vector(targetNearestPlatformPoint.x,
// nearestPlatformToTarget.getHitbox().y - 16);
// }
//
...
// }
//
// if (targetPos.y > player.hitbox.y + 256 && distance < 320 && (target.endLag >
// 0 || !player.falling)
// || distance < 30) {
// release(Player.ATTACK);
// if (targetPos.x > player.center().x) {
...
// }
// if (targetProjectile != null &&
// nextTargetProjectileFrame.overlaps(player.hitbox) && Math.random() > 0.5) {
// tap(Player.PARRY);
// }
// if (targetProjectile != null && projectileDistance < 96) {
// if (player.jumps > 0) {
...
// }
// }
// if ((targetPlatform.getHitbox().overlaps(player.hitbox) ||
// player.touchingStage)
// && targetPlatform.canJumpThrough()
// && targetPos.y > player.center().y + 32) {
// press(Player.DOWN);
// }
// if (distance < 128 && target.damage > 100 && Math.random() > 0.9 ||
// player.charge > 0) {
// press(Player.DOWN);
// press(Player.ATTACK);
// }
// if (player.charge > 30 || player.charge > 0 && (targetPos.x -
// player.center().x)
// / (Math.abs(targetPos.x - targetPos.x)) != player.direction) {
// release(Player.ATTACK);
// }
